...
$filterstr = "SELECT application.applicatipnNo,application.EMIAmount,application.agencyName,application.FOSName,application.FosNumber,application.agentId,application.iAppId,application.isEmiPending,application.bucket,application.customerName,application.branch,application.state,application.customerAddress,application.customerCity,application.customerZipcode,application.loanAmount,applicationfollowup.strEntryDate FROM `application` inner join applicationfollowup on applicationfollowup.iAppId=application.iAppId  " . $where . " and application.isPaid=0 and application.isWithdraw=0 and application.isDelete='0'  and  application.istatus='1' GROUP BY application.iAppId order by STR_TO_DATE(applicationfollowup.strEntryDate,'%d-%m-%Y') DESC";
...
if (mysqli_num_rows($result1) > 0) {
    ...
    while ($rowfollowup = mysqli_fetch_array($result1)) {
        $query = mysqli_fetch_array(mysqli_query($dbconn, "SELECT MAX(Total) as RowCount FROM (SELECT COUNT(*) AS Total FROM applicationfollowup " . $where . " GROUP BY iAppId) AS Results"));
    ...
    $i = 1;
    $filterstr = "SELECT DISTINCT(application.iAppId),applicationfollowup.strEntryDate,application.applicatipnNo,application.EMIAmount,application.agencyName,application.FOSName,application.FosNumber,application.agentId,application.iAppId,application.isEmiPending,application.bucket,application.customerName,application.branch,application.state,application.customerAddress,application.customerCity,application.customerZipcode,application.loanAmount FROM `application` inner join applicationfollowup on applicationfollowup.iAppId=application.iAppId  " . $where . " and application.isPaid=0 and application.isWithdraw=0 and application.isDelete='0'  and  application.istatus='1' GROUP BY application.iAppId order by STR_TO_DATE(applicationfollowup.strEntryDate,'%d-%m-%Y') DESC";
    ...
    while ($row = mysqli_fetch_array($resultFilter)) {
        ...
        $mainData = $row['applicatipnNo']
                ...
                . "\t" . $isEmiPending;
        $subData = array();
        $queryfollowup = mysqli_query($dbconn, "select dispoType,followupDate,PTPDate,mainDispoId,subDispoId,remark,strEntryDate from applicationfollowup " . $where . " and  iAppId ='" . $row['iAppId'] . "' order by STR_TO_DATE(applicationfollowup.strEntryDate,'%d-%m-%Y %H:%i') DESC");
        $iCounter = 0;
