...
if ($_POST['action'] == 'ListUser') {
    ...
    if ($_REQUEST['EmployeeId'] != NULL && isset($_REQUEST['EmployeeId'])) {
        ...
        $filterstr = "SELECT * FROM `tldeshboardmaster` " . $where . $whereEmp . "  and isDelete='0'  order by tlDeshboardId asc";
    } else {
        $filterstr = "SELECT * FROM `tldeshboardmaster` " . $where . " and elisionloginid='" . $_SESSION['elisionloginid'] . "'  and isDelete='0'  order by qualityId asc";
    }
    if ($_REQUEST['EmployeeId'] != NULL && isset($_REQUEST['EmployeeId'])) {
        $whereEmp.=" and elisionloginid='" . $_REQUEST['EmployeeId'] . "'";
        $countstr = "SELECT count(*) as TotalRow FROM `tldeshboardmaster` " . $where . $whereEmp . " and isDelete='0'";
    } else {
