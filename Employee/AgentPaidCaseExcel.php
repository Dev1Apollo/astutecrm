...
$filterstr = "SELECT applicatipnNo,bucket,customerName,branch,state,customerAddress,customerCity,customerZipcode,paid_amount,"
        . " loanAmount,EMIAmount,agencyName,FOSName,FosNumber,PaidDate,agentId,iAppId FROM `application` 
        inner join application_payment_history on  application.iAppId=application_payment_history.application_id  
        " . $where . $whereEmp . " and application.isPaid=1 and application.isDelete='0'  and  application.istatus='1' order by applicatipnNo asc";
...
if (mysqli_num_rows($result1) > 0) {
    ...
    while ($row = mysqli_fetch_array($result1)) {
        ...
        // $filterFollowUp = mysqli_fetch_array(mysqli_query($dbconn, "Select followupDate,PTPDate,dispoType,mainDispoId,subDispoId,strEntryDate,remark from applicationfollowup where iAppId='" . $row['iAppId'] . "' ORDER BY iAppLogId DESC LIMIT 1"));
    ...
    header('location:AgentPaidCase.php');
}
exit;
