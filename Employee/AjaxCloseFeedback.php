...
if ($_POST['action'] == 'ListUser') {
 ...
 $filterstr= "SELECT *, (select empname from employee where employee.employeeid=onlinefeedback.complainBy and isDelete=0)as complainByName,(select categoryName FROM `feedbackcategory` where isDelete='0' and istatus='1' and feedbackCategoryId=onlinefeedback.feedbackCategoryId)as categoryName FROM onlinefeedback WHERE 1=1 ";

 $action = $_SESSION['Designation'];

    switch ($action) {
        case '6'://Central Manager
            $filterstr .= " and onlinefeedback.complainBy= ".$_SESSION['EmployeeId']."  and status = '3' ORDER BY feedbackId desc"; 
            break;
        case '3'://Process Manager
            $filterstr .= " and (onlinefeedback.complainBy= ".$_SESSION['EmployeeId']." or onlinefeedback.agentId = ".$_SESSION['EmployeeId']." or  onlinefeedback.PMId = ".$_SESSION['EmployeeId'].") and status = 3 ORDER BY feedbackId desc"; 
            break;
         case '2'://Assistant Manager
            $filterstr .= " and (onlinefeedback.complainBy= ".$_SESSION['EmployeeId']." or onlinefeedback.agentId = ".$_SESSION['EmployeeId']." or  onlinefeedback.AMId = ".$_SESSION['EmployeeId'].") and status = 3 ORDER BY feedbackId desc"; 
            break;
          case '7'://HR
            $filterstr .= "and (onlinefeedback.complainBy= ".$_SESSION['EmployeeId']." or onlinefeedback.agentId = ".$_SESSION['EmployeeId']." ) and status = 3 ORDER BY feedbackId desc"; 
            break;
          case '4'://Team Leader
            $filterstr .= " and (onlinefeedback.complainBy= ".$_SESSION['EmployeeId']." or onlinefeedback.agentId = ".$_SESSION['EmployeeId']." or  onlinefeedback.tLId = ".$_SESSION['EmployeeId'].") and status = 3 ORDER BY feedbackId desc"; 
            break;
          case '1'://QA
            $filterstr .= "and (onlinefeedback.complainBy= ".$_SESSION['EmployeeId']." or onlinefeedback.agentId = ".$_SESSION['EmployeeId'].") and status = 3 ORDER BY feedbackId desc"; 
            break;
