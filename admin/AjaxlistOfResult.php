...
FROM `employee` e inner join employeedesignation ed on e.employeeid=ed.iEmployeeId inner join  examassigneduser on examassigneduser.userId=e.employeeid where ed.iDesignationId in (4,1,5,9,12,18) and 
     e.istatus=1 and examassigneduser.examId='".$_POST['examId']."'";
     // e.isDelete=0 and
     $resultfilter = mysqli_query($dbconn, $filterstr);
     $rowfilter = mysqli_fetch_array($resultfilter);

    // $filterstr = "SELECT * FROM `questionanswer` " . $where . "  and isDelete='0' and istatus='1' order by questionId desc"; and e.isDelete=0

    $countstr = "SELECT count(*) as TotalRow FROM `employee` e inner join employeedesignation ed on e.employeeid=ed.iEmployeeId where ed.iDesignationId=5  and e.istatus=1 and e.employeeid in (SELECT userId FROM `examassigneduser` where examassigneduser.examId='".$_POST['examId']."')";
        ...
        <table class="table table-bordered table-hover center table-responsive" width="100%" id="tableC">
            ...
            <thead class="tbg">
                ...
                <tr>
                    ...
                    <th class="all">Total Question</th>
                    <th class="all">Attempted Question</th>
                    <th class="all">Unattempted Question</th>
                    <th class="all">Right answer</th>
                    <th class="all">Wrong Answer</th>
                    <th class="all">Obtain Marks</th>
                    <th class="all">Passing Marks</th>
