...
ob_start();                
 $qry="SELECT * FROM `employee`  where 1=1 and isDelete='0'  and  istatus='1' order by empname asc";
    ...
    // $Process = mysqli_fetch_array(mysqli_query($dbconn, "SELECT * FROM `processmaster`  where isDelete='0'  and  istatus='1' and processmasterid='" . $row['iProcessid'] . "' "));
                $processname = "";
                            if($row['iProcessid'] != 0){
                                ...
                                $Process =mysqli_query($dbconn, "SELECT * FROM `employeeprocess` INNER JOIN processmaster ON employeeprocess.iProcessId=processmaster.processmasterid WHERE iEmployeeId='".$row['employeeid']."'");
                                while ($rowProcess=mysqli_fetch_array($Process)) {
                                    $processname.=$rowProcess['processname'].",";
                                }
                            }   
    echo $value3="\t" . rtrim($processname,",")."\t".$row['elisionloginid'];
