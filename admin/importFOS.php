...
if ($_POST['action'] == 'importFOS') {
    $service_url='https://astutemanagement.co.in/herocollections/Employee/getFOS.php';
    $hashKey='astuteCRM';
    ...
    if(count($res_array)>0){
        $deleteAgency=$connect->deleterecord($dbconn,"fosuser");
        if ($res_array['success']==1) {
            foreach ($res_array['foslist']  as $key=>$value) {               
                    
                $strdata="insert into fosuser values('".implode('\',\'',$value)."')";
